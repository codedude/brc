...
func main() {
	...
	go mergeMaps(stopChan, mergeChan, stationMap)
	...
	// err := ReadFileFast("samples/measurements-10000-unique-keys.txt", mergeChan, chunkSize, n_threads)
	// err := ReadFileFast("samples/measurements-10.txt", mergeChan, chunkSize, n_threads)
	err := ReadFileFast("samples/data-1b.txt", mergeChan, chunkSize, n_threads)
	close(mergeChan)
	if err != nil {
		fmt.Println(err)
	}
